document.addEventListener('DOMContentLoaded', function() {
    // Selectores para la página de detalle (si la tienes)
    const btnAgregar = document.getElementById('btnAgregar');
    const btnComprar = document.getElementById('btnComprar');
    
    // Selectores para la página principal y la vista previa del carrito
    const botonesAgregar = document.querySelectorAll('.btnAgregarItem');
    ...
    // Manejador de evento para los botones "Agregar al carrito" en la lista de productos
    if (botonesAgregar.length > 0) {
        botonesAgregar.forEach(boton => {
            boton.addEventListener('click', function() {
                const card = boton.closest('.oferta.card');
